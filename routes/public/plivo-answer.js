...
router.get('/answer', async (req, res) => {
  ...
  try {
    ...
    // Best-effort like the audit update above: a failed log write must not cost
    // the customer their call, so swallow and carry on to the bridge XML.
    try {
      await plivoLog.markAnswered(claims.jci, req.query.CallUUID || null, record);
    } catch (err) {
      logger.warn({ jci: claims.jci, err: err && err.message }, 'plivo answer: markAnswered failed (returning XML anyway)');
    }
    ...
    /*
     * Plivo is holding this request open waiting for call-control XML, so an
     * unexpected throw must never become a 500 — that drops the live leg. Same
     * empty <Response/> the bad-token path returns: no bridge, but a clean call.
     */
    logger.error({ err: e && e.message }, 'Plivo answer: unexpected failure · returning empty Response');
    if (!res.headersSent) xml('<?xml version="1.0" encoding="UTF-8"?>\n<Response></Response>');
  }
});
...
async function webAnswer(req, res) {
  ...
  try {
    ...
    // Best-effort like the audit update above — never let a log write cost the call.
    try {
      await plivoLog.markRinging(resolved.jci, src.CallUUID || null);
      await plivoLog.setRecordingRequested(resolved.jci, record);
    } catch (err) {
      logger.warn({ jci: resolved.jci, err: err && err.message }, 'plivo web-answer: call-log write failed (returning XML anyway)');
    }
    ...
    /*
     * Same contract as /answer: Plivo is waiting on call-control XML, so an
     * unexpected throw must not become a 500. <Hangup/> is this function's own
     * "cannot proceed" reply (see the unknown-dialId path above) — a clean end,
     * not a dropped leg.
     */
    logger.error({ err: e && e.message }, 'plivo web-answer: unexpected failure · hanging up');
    if (!res.headersSent) xml('<?xml version="1.0" encoding="UTF-8"?>\n<Response><Hangup/></Response>');
  }
}
...
async function recordingCallback(req, res) {
  try {
    ...
    // a lost recording URL, which the log records; a 500 would be a retry loop.
    logger.error({ err: e && e.message }, 'Plivo recording-callback: store failed · acking 200 anyway');
    if (!res.headersSent) res.status(200).type('text/plain').send('ok');
  }
}
